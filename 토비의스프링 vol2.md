...
* DispatcherServlet은 그 자체로 스프링 컨텍스트에 등록되는 빈이 아니므로 DI가 일어나는 것은 아님, 하지만 마치 DI가 적용되는 것처럼 서블릿 애플리케이션 컨텍스트의 빈을 가져와 사용
...
* 제네릭스와 매핑정보 상속을 이용해 컨트롤러 작성 480p
...
6장
***
* 테스트 컨텍스트 프레임워크를 사용하면 컨텍스트 캐싱을 통해 테스트 성능을 향상 시킬 수 있음
* 테스트 코드는 독립적으로
* 테스트 @Transactional aop가 적용된것처럼 동작, 기본 롤백으로 동작, @Rollback(false) 롤백이 안되게 바꿀수있음
...
* 테스트에서 사용할 db 데이터 사용시 DBUnit 사용
  - DbataSourceUtils 사용 하여 트랜잭션 커넥션을 가져올수 있
