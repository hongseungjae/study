...
* 하지만 엘라스틱 서치는 동의어나 유의어를 활용한 검색이 가능하며, 비정형 데이터의 색인과 검색이 가능하고, 역색인 지원으로 매우 빠른 검색이 가능합니다.
  - 역색인은 키워드로 로우,도큐먼트를 검색할 수 있음

### 옵티마이저(Optimizer)에 대해 아는대로 말해주세요.
* dbms는 쿼리를 받으면 쿼리로 어떻게 실행되야할지 최적의 실행계획이라는 것을 세우게됩니다. 이런역할을 옵티마이저가 하게 됩니다.

### DB 튜닝(Tuning)이 무엇인지 그리고 튜닝의 3단계에 대해 설명해주세요.
1. DB 설계 튜닝(모델링 관점) , 데이터파일, 테이블 스페이스 설계
2. DBMS 튜닝으로 buffer나 캐시 크기
3. 쿼리튜닝을 할 수 있습니다. 실행계획을 보면서 인덱스 타는지 안타는지 보면서 튜닝을 할 수 있습니다.
...
* 클러스터링 : 러 개의 DB를 수평적인 구조로 구축하는 방식, Fail Over 시스템을 구축하기 위해, 동기식성데이터 일관성
장점
...
* 리플리케이션 : 수직적인 구조(Master-Slave)로 구축하는 방식
장점
