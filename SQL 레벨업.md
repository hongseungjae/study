...
**레코드에 순번 붙이기**
SQL은 전통적으로 순번을 다루기 위한 기능 없음 -> 관계 모델의 이론때문 -> 실질적으로 의미가 없는 순번은 엔티티의 속성으로 보지 않으므로 -> 레코드에 적당한 순번을 붙여야하는 경우 많음 -> 이에 SQLl은 순서와 순번을 다루기 위한 기능들을 추가하고 있음

윈도우 함수(ROW_NUMBER)나 상관 서브쿼리 사
