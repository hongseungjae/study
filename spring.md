...
* 정리 : 빈 스코프에서 싱글톤은 스프링 프레임워크가 한 번 생성하여 관리를 하게 됩니다. 한번 생성하기 때문에 메모리 이점이나, 객체 생성에 대한 오버헤드를 줄일수있습니다. 그리고 이 인스턴스를 공유하기때문에 멀티쓰레드 환경에서 동기화문제에 주의해야합니다. 

프로토타입은 매번 요청마다 새로운 인스턴스를 생성합니다. 따라서 각각 독립적으로 사용되어 멀티쓰레드환경에서 동기화문제에 대해서 안전하게 사용가능하지만, 매번 객체생성을 함으로 오버헤드가 존재합니다.
...
## 무상태성에 대해 설명하시오.
무상태성(statelessness)은 상태를 가지지 않는 것을 의미합니다.

웹 애플리케이션에서 무상태성은 HTTP 프로토콜의 특성을 따르는데, HTTP 프로토콜은 클라이언트와 서버 간의 상태를 유지하지 않고 요청(Request)과 응답(Response)을 주고받습니다. 이러한 특성을 활용하여 웹 애플리케이션도 무상태성을 유지하는 것이 좋습니다.

무상태성은 웹 애플리케이션에서 여러 이점을 가져옵니다. 예를 들어, 서버의 확장성을 높일 수 있습니다. 서버의 부하를 분산시키기 위해 서버를 추가하거나 서버의 스케일 업(Scale-up)을 하면서 무상태성을 유지하면, 서버 간의 세션 공유(Session Sharing)를 할 필요가 없으므로, 더 많은 클라이언트 요청(Request)을 처리할 수 있습니다.

Spring 프레임워크는 무상태성을 유지하기 위해 다양한 방법을 제공합니다. 예를 들어, 스프링의 DispatcherServlet은 클라이언트로부터 요청(Request)을 받으면, 해당 요청(Request)을 처리하기 위해 새로운 객체를 생성합니다. 이를 통해 매 요청(Request)마다 새로운 객체가 생성되어 무상태성을 유지할 수 있습니다. 또한, 스프링은 HTTP 세션을 사용하여 클라이언트의 상태 정보를 저장할 수도 있습니다. 이 경우에도 세션을 사용하지 않는 경우보다 무상태성을 유지하기는 어렵지만, 세션을 사용하면서도 상태 정보를 최소화하여 무상태성을 유지할 수 있습니다.
